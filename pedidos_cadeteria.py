...
for fm in data:
    bot.sendMessage(798817070, '#MOTO'+ str(fm[0]) + 
        '\n -- Fecha: '+ str(fm[15]).split(' ')[0] +' -- '+
        '\nInicia en: \n'+ str(fm[1].encode('ascii',  'ignore').decode('ascii')) +
            ' ('+ str(fm[2].encode('ascii',  'ignore').decode('ascii')) +
            ')\nPreguntar por: '+ str(fm[3].encode('ascii',  'ignore').decode('ascii')) +
        '\n\nDestino: \n'+ str(fm[4].encode('ascii',  'ignore').decode('ascii')) +
            ' ('+ str(fm[5].encode('ascii',  'ignore').decode('ascii')) +
